...
2. 构建并启动 Rust API、Rust worker 与前端服务
   ...
   密钥文件必须恰好 32 字节，并与数据库备份分开保存。已有明文集成凭据的部署需要先保持服务停止并按 [安全说明](docs/security.md) 显式迁移；新安装可直接启动。
...
```bash
cargo run --bin api -- --host 127.0.0.1 --port 8000 --project-root . --secret-key-file secrets/paper-scanner.key
```
...
- `--secret-key-file`：必填的原始 32 字节部署密钥文件；用于认证并解密数据库中的集成凭据
...
API、索引器和调度任务启动时会读取 `runtime_settings`。秘密配置使用版本化认证密文保存，响应只返回配置状态与固定掩码；Docker Compose 另将密钥挂载到 `/run/secrets/paper_scanner_key`，密钥不进入 `./data`。
