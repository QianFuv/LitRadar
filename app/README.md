...
四个工作区复用同一套桌面固定侧栏、移动抽屉、sticky 工具栏、内部文章滚动区和安全区留白。侧栏顶部品牌栏与“检索”“收藏”“周报”“征稿”四列导航固定，数据库选择或收藏夹管理控件也保持可见，下方筛选、收藏夹和期刊列表独立滚动。侧栏统一使用 16px 外层留白，列表不重复增加水平 padding，数据库标签和选择器同行。每个图标入口都提供可访问名称、悬停提示和当前页面语义。
