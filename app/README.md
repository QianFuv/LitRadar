...
认证完成后，所有受保护页面右下角都会显示全局用户菜单；管理员还会看到管理面板入口。菜单同时提供跟随系统、浅色和深色三种主题偏好，并通过 Radix Dropdown Menu 处理键盘导航、Escape、点击外部关闭和焦点归还。菜单位置与页面底部留白会考虑设备安全区。设置中心使用 Radix Dialog：桌面为双栏弹窗，移动端为全屏单列，文献追踪和通知分类共享同一份未保存草稿，并在关闭或离开追踪分类组前要求确认。
