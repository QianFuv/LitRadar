...
备份实现位于 `crates/ps-storage/src/backup.rs`。测试必须使用临时 WAL 数据库、临时状态文件和独立恢复根目录，覆盖 online backup、清单/hash/`quick_check`、未知版本、缺失/额外文件、活动心跳、替换失败回滚与 CLI 显式确认；不得读取或恢复仓库真实 `data/`。外部调用语义见 [备份与离线恢复](backup.md)。
...
| `ps-storage` | SQLite auth/index 存储访问、版本化备份与离线恢复 |
...
先生成仅供本机开发使用的原始 32 字节密钥；`secrets/` 已被 Git 忽略：
...
所有会读取集成凭据的后端入口都必须显式传入该文件。测试只使用临时密钥和临时数据库，不读取仓库的真实 `data/auth.sqlite` 或本机开发密钥。

### Rust API

```bash
cargo run --bin api -- --secret-key-file secrets/paper-scanner.key
```
...
```bash
RUST_LOG=error cargo run --bin api -- --secret-key-file secrets/paper-scanner.key
RUST_LOG=ps_api=debug,tower_http=debug cargo run --bin api -- --secret-key-file secrets/paper-scanner.key
```
...
```bash
cargo run --bin worker -- --secret-key-file secrets/paper-scanner.key --interval-seconds 30
```
...
```bash
cargo run --bin scheduler -- --secret-key-file secrets/paper-scanner.key validate
cargo run --bin scheduler -- --secret-key-file secrets/paper-scanner.key run-once 1
cargo run --bin scheduler -- --secret-key-file secrets/paper-scanner.key dry-run-once 1
```
...
```bash
cargo run --bin index -- --secret-key-file secrets/paper-scanner.key --file english_journals.csv --update
cargo run --bin index -- --secret-key-file secrets/paper-scanner.key --file cnki_journals.csv --resume --issue-batch 10
```
