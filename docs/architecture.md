...
唯一二进制在分发子命令前安装一个进程级 tracing subscriber。所有 crate 只产生结构化事件，不各自初始化 subscriber 或写应用日志文件：
...
HTTP 外层先移除不受信的 `X-Request-Id`，再生成并返回服务器 UUID；请求内的异步和 blocking 工作重新进入同一 span。成功健康检查、静态资源与前端文档不产生逐请求终态事件。调度 span 使用 `run_id` 关联任务，规范子进程通过 `parent_run_id` 回连父运行；子进程 `stderr` 直接继承到同一容器 sink，`stdout` 仍保留给结构化业务结果。
...
- 未设置 `LITRADAR_BUNDLED_META_DIR` 的本地构建不执行受管准备；`data/meta` 目录缺失时索引明确失败，目录存在但没有选中的 CSV 时返回 `skipped`。
...
目录验证在 Provider 请求前拒绝未知列、重复/非法 `catalog_id`、非法 ISSN、重复别名和不规范文本。路由来自 `auth.sqlite.runtime_settings.index_provider_routes`；内容库和目录都不知道实际 Provider。
...
内容库只向 API 提供规范 `ArticleLocator`。详情、摘要页和全文分别按三个运行时顺序选择声明了相应能力的 Provider：
...
上游目的地不会出现在 `/access`、文章响应或索引库中。动作调用也不更新文章、outbox、checkpoint、认证会话或文件缓存。Provider 注册携带精确的运行时跳转域名 allowlist，API 不按 Provider 名称硬编码域名。
...
- `runtime_settings`：外部元数据 key 池、索引/在线 Provider 路由、CORS、MCP 和 Cookie 策略
