...
如果用户表非空，bootstrap 必须拒绝运行。丢失管理员密码时，应使用另一个已认证管理员的重置接口或恢复经过验证的数据库备份；不要删除用户、手工降低 schema 版本或修改 `is_admin` 来绕过流程。

`admin backup` 只创建版本化目录清单与数据组件，不包含部署密钥。恢复必须显式传入 `--confirm-restore`，先验证全部文件/hash/schema/SQLite 完整性，再拒绝最近 90 秒内有 API 或 worker 心跳的目标，最后通过同文件系统替换与逆序回滚完成。备份与匹配密钥必须分开保存；详见 [备份与离线恢复](backup.md)。
