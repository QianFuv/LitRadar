...
`admin backup` 创建版本化、可独立验证的目录备份。部署密钥永远不在备份中，必须单独保存。命令语法见 [CLI 参考](../reference/cli.md)。
...
需要严格时间点一致性时，先停止 API、worker、索引和投递写入。
...
1. 在只读副本上再次运行 `admin backup verify`。
2. 停止所有读写目标数据的服务。
3. 等待 API/worker 最近心跳超过 90 秒。
4. 保留当前目标数据的独立回滚副本。
5. 执行带显式确认的恢复。
...
恢复开始前会再次验证整个备份，并在替换前后检查 `service_heartbeats` 和旧 `scheduler_workers`。
