...
| 内容索引库  |                     5 | 新建/验证精确 v5；精确 v4 原子迁移到 v5 |
| 索引控制库  |                     1 | 可删除后按 v1 重建                    |
...
- 表、列、索引和 `user_version` 精确匹配的 v4，随后在一个事务内迁移到 v5；
- 表、列、索引和 `user_version` 精确匹配的 v5。

非空 v0 以及 v1–v3 返回 `IndexRebuildRequired`，文件保持字节不变。未来版本也在业务访问前拒绝。v4 迁移只新增期刊身份键表及索引，并从已有期刊的 catalog ID 和 ISSN 字段播种身份键；期刊、期次、文章、投影、outbox 和稳定 ID 均保持不变。不要手工修改 `user_version` 或拼接表结构。
...
旧 v1–v3 文章 ID、收藏和 tracking 引用不会迁移或重映射；精确 v4 到 v5 的迁移不重映射这些 ID。
...
内容 schema 只有以下八个内容对象（另有这些对象所需的辅助索引）。Provider 路由、名称、上游 ID、URL、checkpoint、lease、运行统计、Cookie 和会话都不允许出现在该库。
...
内容 v5 不依赖外部 `simple` tokenizer 创建 schema。
...
v1–v3 的破坏性重建不会重映射旧 favorite 的 article ID；精确 v4 到 v5 的迁移保留 ID。无法解析的旧引用由运维人员或用户清理。
