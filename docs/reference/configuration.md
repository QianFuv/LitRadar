...
管理员提交包含无效 Origin 的 `PUT /api/admin/runtime-settings` 时，API 在写入前返回 `400`，同一请求中的其他字段也不会保存。有效设置不会热加载，在 API 下次启动时生效。

旧版本或库外修改留下的无效 Origin 行会让新 API 在绑定端口前以明确配置错误拒绝启动，不会忽略、自动删除或降级该策略。升级前应通过当前运行的管理 API 修正；若新版本已经无法启动，应在维护窗口恢复可验证备份或纠正该非秘密行，再重新启动。
