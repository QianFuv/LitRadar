...
| `GET` | `/api/articles/{article_id}/access`   | 本地计算详情、摘要页和全文能力   |
| `GET` | `/api/articles/{article_id}/detail`   | 在线解析并 307 跳转到详情页      |
...
它不请求上游，也不返回 Provider 名称或目的 URL。响应始终包含 `detail`、`abstract_page`、`fulltext` 三个动作，每个动作只有 `available`、`label`、`requires_login` 和可选 `message`。

动作路由在用户调用时才加载 Provider-neutral `ArticleLocator` 并在线解析。详情、摘要页和全文分别使用独立的有序 Provider 列表；未注册或未声明能力的名称被忽略。超时、未找到、临时失败、无效结果或需要认证会继续尝试后续 Provider。全部失败后才返回 `404`，或在只剩认证阻断时返回结构化 `428 article_access_authentication_required`。
...
前端只生成 LitRadar 自己的 `/detail`、`/abstract`、`/fulltext` URL，不直接使用索引字段或上游 permalink。DOI 仍是规范文章标识，可用于复制和生成引文。Provider 接入规范见[索引与 Provider 契约](index-provider-contract.md)。
