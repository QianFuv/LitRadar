...
本地默认地址为 `http://localhost:8000`，REST 路径统一以 `/api` 开头。支持两种认证方式：

| 使用场景                      | 凭据                                            |
| ----------------------------- | ----------------------------------------------- |
| 同源 Web 前端                 | 登录后由后端设置的 `ps_session` HttpOnly Cookie |
...
| 方法  | 路径                                  | 作用                                 |
| ----- | ------------------------------------- | ------------------------------------ |
| `GET` | `/api/health`                         | API 存活状态                         |
| `GET` | `/api/health/worker`                  | 最近 90 秒内是否存在持久 worker 心跳 |
...
公开注册始终要求有效邀请码，且只能创建非管理员。首个管理员必须在本机通过 `admin bootstrap` 创建，API 不提供远程引导端点。新密码至少为 12 个 Unicode 字符。

CNKI 会话按 Paper Scanner 用户隔离；状态接口只返回安全元数据，不返回 token 或 Cookie 值。
...
| `GET`            | `/api/admin/scheduler/status`                | 调度游标、worker 心跳与近期运行 |
