...
5. 按规范化 DOI 分批请求 OpenAlex 和 Semantic Scholar。
...
底层客户端支持可选 `from-pub-date`，但当前 live index 调用传入空值；`index --update` 的含义是对写入前后快照生成变更清单，不是缩小 Crossref 请求日期窗口。
...
实现把 DOI 切成最多 500 个 ID 的批次。官方 Graph API schema 将 batch 上限定为 500；官方产品页给出的 introductory API-key 限额为 1 request/second：
...
这让默认多进程执行近似共享 1 RPS，但不是跨主机的分布式限流器。
