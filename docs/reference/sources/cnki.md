...
CNKI 元数据索引、CNKI 在线摘要页和浙江图书馆全文是三个独立运行时边界。它们共享[规范文章契约](../index-provider-contract.md)，不通过索引 provenance 或持久 URL 互相绑定。
...
步骤中的“候选详情”只是 CNKI adapter 内部用于核对文章身份的上游页面，不构成额外的公开在线 capability。LitRadar 前端的文章详情弹窗读取本地内容库；在线动作只暴露“查看摘要页”。
...
当前没有代理池运行设置。`--workers` 和 `--issue-batch` 只控制 CNKI adapter 内的文章解析工作，`--processes` 控制同一目录的 journal worker；默认值和内存边界见[CLI 参考](../cli.md)。
