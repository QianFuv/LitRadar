...
mod tests {
        ...
        tokio::time::timeout(Duration::from_secs(1), async {
            loop {
                let now = SystemTime::now()
                    ...
                    .as_secs_f64();
                if litradar_storage::has_recent_service_heartbeat(&auth_db_path, now, 90.0)
    ...
    fn secure_cookie_startup_gate_fails_closed_and_accepts_database_setting() {
        ...
        let storage_config = litradar_storage::StorageConfig::from_project_root(temp_dir.path());
        let secret_codec =
            litradar_storage::SecretCodec::load(secret_key_file).expect("secret key should load");
    ...
    fn runtime_origin_startup_rejects_legacy_values_without_panicking() {
        for (field, value, expected_error) in [
            ...
            let temp_dir = tempfile::tempdir().expect("temporary project should be created");
            let storage = litradar_storage::StorageConfig::from_project_root(temp_dir.path());
    ...
    fn router_startup_rejects_missing_wrong_and_tampered_secret_material() {
        ...
        let codec =
            litradar_storage::SecretCodec::load(&correct_key).expect("correct key should load");
        litradar_storage::upsert_runtime_settings(
            storage.auth_db_path(),
            &codec,
            &HashMap::from([(
    ...
    async fn backend_fixtures_create_authenticated_router_and_index_database() {
        ...
        let database_names = litradar_storage::list_index_database_names(backend.storage_config())
            .expect("database names should load");
        let journal = litradar_storage::get_journal(
            backend.storage_config(),
            Some(&index_database.db_name),
            index_database.journal_id,
        )
        .expect("journal should load");
        let issue = litradar_storage::get_issue(
            backend.storage_config(),
            Some(&index_database.db_name),
            index_database.issue_id,
        )
        .expect("issue should load");
        let article = litradar_storage::get_article(
            backend.storage_config(),
            Some(&index_database.db_name),
            index_database.article_id,
        )
        .expect("article should load");
        let articles = litradar_storage::list_articles(
            backend.storage_config(),
            Some(&index_database.db_name),
            &litradar_storage::ArticleListParams {
    ...
    async fn auth_routes_cover_registration_login_tokens_invites_and_logout() {
        ...
        assert!(session_cookie.starts_with("litradar_session="));
    ...
    async fn access_token_route_enforces_authentication_before_creation_validation() {
        ...
        for index in existing_count..ACCESS_TOKEN_ACTIVE_LIMIT {
            let row = litradar_storage::insert_personal_access_token(
    ...
    async fn scheduler_admin_routes_cover_user_invite_stats_settings_tasks_and_announcements() {
        ...
        let persisted_task =
            litradar_storage::get_scheduled_task(backend.storage_config().auth_db_path(), task_id)
                .expect("fixture task should load")
                .expect("fixture task should exist");
        litradar_storage::enqueue_scheduled_runs(
            backend.storage_config().auth_db_path(),
            &persisted_task,
            &[(scheduler_now as i64).div_euclid(60) * 60],
        )
        .expect("fixture run should queue");
        let fixture_claim = litradar_storage::claim_ready_scheduled_runs(
            backend.storage_config().auth_db_path(),
            "fixture-worker",
            scheduler_now,
            90.0,
    ...
    async fn tracking_routes_cover_status_and_notification_settings_validation() {
        ...
        .await;
        let subscribers = litradar_storage::list_notification_subscribers(
            backend.auth_db_path(),
    ...
    async fn cnki_routes_cover_replay_session_status_and_clear_without_secret_leaks() {
        ...
        .await;
        let storage_session = litradar_storage::get_cnki_session_status(
            backend.auth_db_path(),
            backend.secret_codec(),
            user.user_id(),
        )
    ...
    async fn cnki_routes_use_live_fixture_without_replay_mode() {
        ...
        .await;
        let session_data = litradar_storage::get_cnki_session_data(
            backend.auth_db_path(),
            backend.secret_codec(),
            user.user_id(),
        )
