...
use litradar_domain::{ArticleId, FavoriteAdd, OkResponse, UserId, UserResponse};
...
type InnerMcpService = StreamableHttpService<LitRadarMcp, LocalSessionManager>;
...
pub(crate) fn service(config: &ApiConfig, state: ApiState) -> AuthenticatedMcpService {
    ...
    let inner = StreamableHttpService::new(
        move || Ok(LitRadarMcp::new(mcp_state.clone())),
...
impl LitRadarMcp {
    #[tool(
        name = "list_databases",
        description = "List available LitRadar SQLite databases."
    )]
    async fn list_databases(&self) -> Result<CallToolResult, ErrorData> {
        self.run_index_tool(move |storage| litradar_storage::list_index_database_names(&storage))
            .await
    }

    #[tool(
        name = "list_areas",
        description = "List research areas for the selected LitRadar database."
    )]
    async fn list_areas(
        &self,
        ...
        self.run_index_tool(move |storage| litradar_storage::list_areas(&storage, db.as_deref()))
            .await
    }
    ...
    async fn list_years(
        ...
        self.run_index_tool(move |storage| litradar_storage::list_years(&storage, db.as_deref()))
            .await
    }

    #[tool(
        name = "list_journal_options",
        description = "List journal filter options for the selected LitRadar database."
    )]
    ...
    #[tool(
        ...
        description = "Search articles in the LitRadar index."
    )]
    ...
    #[tool(
        ...
        description = "Get weekly update summaries across all LitRadar databases."
    )]
    async fn get_weekly_updates(&self) -> Result<CallToolResult, ErrorData> {
        self.run_index_tool(move |storage| litradar_storage::get_weekly_updates(&storage))
            .await
    }

    #[tool(
        name = "list_folders",
        description = "List favorite folders for the authenticated LitRadar user."
    )]
    async fn list_folders(
        ...
        self.run_business_tool(move |storage| {
            litradar_storage::list_folders(storage.auth_db_path(), user_id)
        })
        .await
    }
...
impl ServerHandler for LitRadarMcp {
    fn get_info(&self) -> ServerInfo {
        ServerInfo::new(ServerCapabilities::builder().enable_tools().build())
            .with_instructions("Use LitRadar tools to query indexed papers and favorites.")
    }
}
...
fn blocking_mcp_error(_error: BlockingTaskError) -> ErrorData {
    ErrorData::internal_error("LitRadar backend is temporarily unavailable", None)
}
